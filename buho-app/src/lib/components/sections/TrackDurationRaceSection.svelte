<script lang="ts">
    ...
    function rank(names: Set<string>, value: (name: string) => number): RankedTrack[] {
        ...
        function isGreater(a: RankedTrack, b: RankedTrack): boolean {
            if (a.value !== b.value) return a.value > b.value;
            return a.item.localeCompare(b.item) < 0;
        }

        function minIndex(list: RankedTrack[]): number {
            let idx = 0;
        ...
        top.sort((a, b) => b.value - a.value || a.item.localeCompare(b.item));
    ...
    function buildKeyframes(rows: TrackMonthlyDurationData[]): RaceKeyframe[] {
        if (rows.length === 0) return [];

        const names = new Set(rows.map((d) => d.item));
    ...
    async function fetchData() {
        try {
            data = await getTopTracksMonthlyDuration(null, currentDateRange());
            keyframes = buildKeyframes(data);
            colorScale = d3
                .scaleOrdinal<string, string>()
                .domain(Array.from(new Set(data.map((d) => d.item))))
                .range(PALETTE);
    ...
    function updateChartWidth() {
        ...
        chartWidth = Math.max(980, Math.floor(chartContainer.clientWidth - 32));
